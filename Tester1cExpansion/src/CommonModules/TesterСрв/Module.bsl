	...
	Для каждого ВеткаТипа из ДеревоСхемы.Строки Цикл
		...
		Для каждого ЛистТаблиц из ВеткаТипа.Строки Цикл
			...
			Если СокрЛП(ЛистТаблиц.Назначение) = "Основная" Тогда
				...
				ИменаКолонокИсключений = новый СписокЗначений;
				Если нВеткаТипа.ТипМетаданных = "Документ" Тогда				
					ИменаКолонокИсключений.ЗагрузитьЗначения(СписокИсключенийДляДокументов.ВыгрузитьЗначения());
				ИначеЕсли нВеткаТипа.ТипМетаданных = "Справочник" Тогда
					ИменаКолонокИсключений.ЗагрузитьЗначения(СписокИсключенийДляСправочников.ВыгрузитьЗначения());					
				КонецЕсли;
				
				ДополнительныеКолонкиИсключения = РегистрыСведений.Tester_ДопСведенияШаблонныхДанных.ПолучитьИменаКолонокИсключенийДляТаблицы(ЛистТаблиц.ИмяТаблицы);
				Для Каждого ИмяДопКолонки из ДополнительныеКолонкиИсключения Цикл
					ИмяКолИскл = СокрЛП(ИмяДопКолонки);
					Если ЗначениеЗаполнено(ИмяКолИскл) Тогда
						ИменаКолонокИсключений.Добавить(ИмяКолИскл);	
					КонецЕсли;	
				КонецЦикла;	
				...
				ИмяМетодаЗапросаКТаблице = РегистрыСведений.Tester_ДопСведенияШаблонныхДанных.ПолучитьМетодЗапросаКТаблице(ЛистТаблиц.ИмяТаблицы);
				ИмяМетодаЗапросаКТаблице = СокрЛП(ИмяМетодаЗапросаКТаблице);
				Если ЗначениеЗаполнено(ИмяМетодаЗапросаКТаблице) Тогда
					нВеткаОсновнойТаблицы.МетодЗапросаКТаблице = ИмяМетодаЗапросаКТаблице;	
				КонецЕсли;		
			...
			ИначеЕсли СокрЛП(ЛистТаблиц.Назначение) = "ТабличнаяЧасть" Тогда
				...
				ИменаКолонокИсключений = новый СписокЗначений;
				Если нВеткаТипа.ТипМетаданных = "Документ" Тогда				
					ИменаКолонокИсключений.ЗагрузитьЗначения(СписокИсключенийДляТабличныхЧастейДокументов.ВыгрузитьЗначения());
				ИначеЕсли нВеткаТипа.ТипМетаданных = "Справочник" Тогда
					ИменаКолонокИсключений.ЗагрузитьЗначения(СписокИсключенийДляТабличныхЧастейСправочников.ВыгрузитьЗначения());
				КонецЕсли;
				...
				ИмяМетодаЗапросаКТаблице = СокрЛП(ИмяМетодаЗапросаКТаблице);
				Если ЗначениеЗаполнено(ИмяМетодаЗапросаКТаблице) Тогда
					нВеткиТабличныхЧастей.МетодЗапросаКТаблице = ИмяМетодаЗапросаКТаблице;	
				КонецЕсли;			
...
Функция ЭтоСлужебнаяТаблицаРасширенияTester(Знач ИмяТаблицы) ЭКСПОРТ 
	Возврат НЕ (СтрНайти(ИмяТаблицы,"Tester_") = 0 И СтрНайти(ИмяТаблицы,"Kiur_") = 0 И СтрНайти(ИмяТаблицы,"Киур_") = 0);
КонецФункции	
	...
	Для каждого стр из ТаблицыБД Цикл
		Попытка
			...
			ИначеЕсли стр.ТипМетаданных = "РегистрСведений" Тогда
				SQL_Text = "TRUNCATE TABLE [dbo].[" + стр.ИмяТаблицыХранения + "]";
				SQLConnect.Execute(SQL_Text);
				РегистрыСведений[СтрРазделить(стр.ИмяТаблицы,".")[1]].ПересчитатьИтоги();
...
Функция ВыполненаШаблоннаяЗагрузкаXMLДляПостОбработки(Знач ФиксированныеПараметры, МодифицируемыеПараметры) ЭКСПОРТ
	МодифицируемыеПараметры.Вставить("Успешно",Ложь);
	
	ХранилищеXMLСтартовыхДанных = ФиксированныеПараметры.ХранилищеXMLСтартовыхДанных;
	...
	МодифицируемыеПараметры.Вставить("Успешно",Истина);
	Возврат Истина;
КонецФункции

Процедура ВыполненШаблонныйЗапросКТаблицеБД(Знач ФиксированныеПараметры, МодифицируемыеПараметры) ЭКСПОРТ
	МодифицируемыеПараметры.Вставить("Успешно",Ложь);
	
	ТекстВыборкиДополнительныхПолейВТаблицу = МодифицируемыеПараметры.ТекстВыборкиДополнительныхПолейВТаблицу;
	ТекстСоединенийСДопТаблицами = МодифицируемыеПараметры.ТекстСоединенийСДопТаблицами;
	...
	//------> Адаптация под кКонкретную таблицу ------>
	Текст_ЗапросаИтоговойТаблицы = Текст_ЗапросаИтоговойТаблицы + ТекстВыборкиДополнительныхПолейВТаблицу;
	//------> Адаптация под кКонкретную таблицу <------
	
	Текст_ЗапросаИтоговойТаблицы = Текст_ЗапросаИтоговойТаблицы + Символы.ПС + "ИЗ" + Символы.ПС + "	" + ИмяТаблицы + " КАК ОсновнаяТаблица";
	
	//------> Адаптация под кКонкретную таблицу ------>
	Текст_ЗапросаИтоговойТаблицы = Текст_ЗапросаИтоговойТаблицы + ТекстСоединенийСДопТаблицами;
	//------> Адаптация под кКонкретную таблицу <------
	...
	МассивИменИсключений = ИменаКолонокИсключений.ВыгрузитьЗначения();
	Для каждого ИмяКолонки из МассивИменИсключений Цикл
		Если ИтоговаяТаблицаЗначений.Колонки.Найти(ИмяКолонки) <> Неопределено Тогда
...
Функция ВыполненаСтандартнаяСверкаШаблонныхДанных(Знач ФиксированныеПараметры,МодифицируемыеПараметры) ЭКСПОРТ
	...
	Если НЕ ВыполненаПроверкаИдентичностиСоответствийДанных(НеИзменяемыеПараметры, ИзменяемыеПараметры) Тогда
		СообщениеОшибки = ?(ИзменяемыеПараметры.Свойство("СообщениеОшибки",СообщениеОшибки),СообщениеОшибки,"");
		ХранилищаСводныхТаблицРазличий = ?(ИзменяемыеПараметры.Свойство("ХранилищаСводныхТаблицРазличий",ХранилищаСводныхТаблицРазличий),ХранилищаСводныхТаблицРазличий,новый Соответствие);		
		МодифицируемыеПараметры.Вставить("СообщениеОшибки",СообщениеОшибки);
		МодифицируемыеПараметры.Вставить("ХранилищаСводныхТаблицРазличий",ХранилищаСводныхТаблицРазличий);
		Возврат Ложь;
	КонецЕсли;	
...
Функция ВыполненаПроверкаИдентичностиСоответствийДанных(Знач ФиксированныеПараметры, МодифицируемыеПараметры) ЭКСПОРТ
	...
	соотХранилищаСводныхТаблицРазличий = новый Соответствие;
	...
	Для каждого ИмяТаблицы_ВложенноеСоответствие из ТекущиеДанныеБД Цикл
		...
		НеИзменяемыеПараметры = новый Структура;
		НеИзменяемыеПараметры.Вставить("ПерваяТаблица", ТаблицаДанныхШаблона);
		НеИзменяемыеПараметры.Вставить("ВтораяТаблица", ТекущаяТаблицаДанныхБД);
		НеИзменяемыеПараметры = новый ФиксированнаяСтруктура(НеИзменяемыеПараметры);
		...
		Если НЕ КиурСрв.ВыполненаПроверкаНаИдентичностьТаблицЗначений(НеИзменяемыеПараметры, ИзменяемыеПараметры) Тогда
			...
			СводнаяТаблицаРазличий = ?(ИзменяемыеПараметры.Свойство("СводнаяТаблицаРазличий",СводнаяТаблицаРазличий),СводнаяТаблицаРазличий,Неопределено);
			Если ТипЗнч(СводнаяТаблицаРазличий) = Тип("ТаблицаЗначений") Тогда
				хранСводнаяТаблицаРазличий = КиурСрв.СоздатьХранилищеЗначения(СводнаяТаблицаРазличий);
				соотХранилищаСводныхТаблицРазличий.Вставить(ИмяПроверяемойТаблицыБД, хранСводнаяТаблицаРазличий);
			КонецЕсли;	 	
			МодифицируемыеПараметры.Вставить("СообщениеОшибки",СообщениеОшибки);
			//Возврат Ложь;
			Продолжить;
	...
	Если ЗначениеЗаполнено(масИменТаблицНеПрошедшихПроверки) Тогда
		...
		МодифицируемыеПараметры.Вставить("ХранилищаСводныхТаблицРазличий",соотХранилищаСводныхТаблицРазличий);
		МодифицируемыеПараметры.Вставить("СообщениеОшибки",СообщениеОшибки);
		Возврат Ложь;
	КонецЕсли;	
