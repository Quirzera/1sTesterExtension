...
//	КонецЕсли;		
	...
	а = 0;
	//Для основных таблиц - Если это таблица в которых могут быть предопределенные элементы(Справочник, ПВХ и тп.)
	//DELETE запрос где поле имя Предопределенных незаполнено, для тех которые заполнено формируем структуру "Справочник.ИмяСправочник",массив UIDов предопределенных.
	//Если это таблица вкоторой предопределеных не бывает - просто TRUNCATE.
	
	//Для второстпенных таблиц - если предопределенных быть не может - тогда TRUNCATE, если могут - смотрим в структуру.
	//Если там есть что- то DELETE с отбором, иначе Truncate/
